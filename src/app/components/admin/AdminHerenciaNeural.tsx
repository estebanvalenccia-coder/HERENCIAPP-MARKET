...
export function AdminHerenciaNeural(){
 ...
 const [conversationLearning,setConversationLearning]=useState<any>(null);
 ...
 const refresh=useCallback(async()=>{
  ...
  if(conv.status==="fulfilled")setConversationLearning(conv.value);
 ...
 return <div className="space-y-6">
  ...
  {tab==="learning"&&<Panel title="Aprendizaje y patrones" icon={Sparkles}><div className="grid gap-5 xl:grid-cols-2">
   <div className="xl:col-span-2 rounded-2xl border p-5">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
     <div><h3 className="font-black text-lg">Aprendizaje conversacional real</h3><p className="mt-1 text-sm text-muted-foreground">Neural conserva el contexto del chat admin, aprende tus reglas y preferencias, y solo convierte señales de clientes en conocimiento cuando se repiten en conversaciones independientes.</p></div>
     <button onClick={()=>void consolidateConversations()} className="rounded-xl border px-4 py-2 text-xs font-black hover:bg-muted">Consolidar conversaciones</button>
    </div>
    <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
     <Mini label="Turnos admin" value={String(conversationLearning?.stats?.adminTurns||0)}/>
     <Mini label="Aprendido de ti" value={String(conversationLearning?.stats?.ownerLearnings||0)}/>
     <Mini label="Candidatos" value={String(conversationLearning?.stats?.ownerCandidates||0)}/>
     <Mini label="Señales cliente" value={String(conversationLearning?.stats?.customerSignals||0)}/>
     <Mini label="Patrones cliente" value={String(conversationLearning?.stats?.customerPatterns||0)}/>
     <Mini label="Vacíos de conocimiento" value={String(conversationLearning?.stats?.knowledgeGaps||0)}/>
    </div>
    <div className="mt-5 grid gap-3 lg:grid-cols-2">
     <div><b className="text-sm">Reglas y preferencias aprendidas de ti</b><div className="mt-2 space-y-2">{(conversationLearning?.context?.owner||[]).slice(0,10).map((x:any,i:number)=><div key={i} className="rounded-xl bg-emerald-50 p-3 text-sm"><div className="mb-1 text-[10px] font-black uppercase text-emerald-800">{String(x.type||"regla").replace("owner_","")}</div>{x.statement}</div>)}{!(conversationLearning?.context?.owner||[]).length&&<Empty text="Habla con Neural o dile «recuerda que…» para empezar a crear conocimiento propio."/>}</div></div>
     <div><b className="text-sm">Patrones aprendidos de clientes</b><div className="mt-2 space-y-2">{(conversationLearning?.context?.patterns||[]).slice(0,10).map((x:any,i:number)=><div key={i} className="rounded-xl bg-muted/50 p-3 text-sm"><div className="flex items-center justify-between gap-2"><b>{x.topic||"tema"}</b><span className="text-xs">{x.count||0} conversaciones</span></div><p className="mt-1 text-xs text-muted-foreground">{x.summary}</p></div>)}{!(conversationLearning?.context?.patterns||[]).length&&<Empty text={"Un patrón se verifica cuando aparece en "+String(conversationLearning?.stats?.patternThreshold||3)+" conversaciones independientes."}/>}</div></div>
    </div>
   </div>
   <div className="rounded-2xl border p-5"><h3 className="font-black">Demanda aprendida de conversaciones</h3><p className="mt-1 text-sm text-muted-foreground">Las señales individuales siguen siendo provisionales; la repetición entre clientes distintos permite consolidar patrones.</p><div className="mt-4 space-y-2">{(demand?.topics||[]).slice(0,20).map((entry:any,i:number)=>{const key=Array.isArray(entry)?entry[0]:entry?.topic;const rows=Array.isArray(entry)?entry[1]:entry?.rows;return <div key={key||i} className="flex items-center justify-between rounded-xl bg-muted/50 p-3"><span className="font-medium">{key||"sin tema"}</span><b>{Array.isArray(rows)?rows.length:Number(entry?.count||0)}</b></div>})}{!(demand?.topics||[]).length&&<Empty text="Todavía no hay suficiente demanda agregada."/>}</div></div>
   <div className="rounded-2xl border p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="font-black">Patrones de negocio</h3><p className="mt-1 text-sm text-muted-foreground">Descriptivos, no causales. La confianza depende del tamaño de la muestra.</p></div><button onClick={()=>void consolidatePatterns()} className="rounded-xl border px-3 py-2 text-xs font-bold hover:bg-muted">Consolidar</button></div><div className="mt-4 grid grid-cols-2 gap-3"><Mini label="Ventas muestra" value={String(patterns?.sample?.verifiedSales||0)}/><Mini label="Confianza" value={String(patterns?.confidence||"—")}/><Mini label="Clientes recurrentes" value={String(patterns?.repeatCustomers||0)}/><Mini label="Combos detectados" value={String((patterns?.coPurchases||[]).length)}/></div><div className="mt-4 space-y-2">{(patterns?.topProducts||[]).slice(0,8).map((p:any)=><div key={p.id} className="flex justify-between rounded-xl border p-3 text-sm"><span>{p.name}</span><b>{p.units} uds.</b></div>)}</div></div>
   <div className="xl:col-span-2"><MentorPanel/></div>
  </div></Panel>}
  ...
  {tab==="goals"&&<Panel title="Objetivos persistentes" icon={Target}><div className="flex gap-2 mb-5"><input value={goal} onChange={e=>setGoal(e.target.value)} onKeyDown={e=>e.key==="Enter"&&void addGoal()} className="flex-1 border rounded-xl px-4 bg-background" placeholder="Ej. reducir roturas de stock"/><button onClick={()=>void addGoal()} className="px-5 py-3 rounded-xl bg-primary text-primary-foreground font-bold">Añadir</button></div><div className="space-y-3">{goals.map((g:any)=><div key={g.id} className="border rounded-2xl p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><b>{g.text}</b><p className="text-xs text-muted-foreground mt-2">{g.status} · prioridad {g.priority||50}</p></div><div className="flex flex-wrap gap-2"><select value={g.status||"ACTIVE"} onChange={e=>void updateGoal(g.id,{status:e.target.value})} className="rounded-lg border bg-background px-2 py-1 text-xs font-bold"><option value="ACTIVE">Activo</option><option value="PAUSED">Pausado</option><option value="COMPLETED">Completado</option></select><button onClick={()=>void deleteGoal(g.id)} className="rounded-lg border border-red-200 px-2 py-1 text-xs font-bold text-red-700 hover:bg-red-50">Eliminar</button><span className="rounded-lg bg-muted px-2 py-1 text-xs font-bold">{g.progress||0}%</span></div></div><div className="h-2 bg-muted rounded-full mt-3 overflow-hidden"><div className="h-full bg-primary" style={{width:`${Math.max(0,Math.min(100,g.progress||0))}%`}}/></div></div>)}</div></Panel>}
...
function MentorPanel(){
 const [status,setStatus]=useState<any>(null),[items,setItems]=useState<any[]>([]),[topic,setTopic]=useState("mejorar ventas, inventario y experiencia de cliente"),[busy,setBusy]=useState(false);
 const load=useCallback(async()=>{try{const [s,c]=await Promise.all([backendApi.neuralMentorStatus(),backendApi.neuralMentorCandidates()]);setStatus(s);setItems(c.candidates||[])}catch{setStatus({connected:false,enabled:false});setItems([])}},[]);
 useEffect(()=>{void load()},[load]);
 const teach=async()=>{if(!topic.trim()||busy)return;setBusy(true);try{const r=await backendApi.neuralMentorTeach(topic.trim(),true);if(r?.ok)toast.success("Sesión Mentor completada: aprendizaje provisional guardado");else toast.error(r?.skipped||"No se pudo entrenar");await load()}catch(e:any){toast.error(e.message||"No se pudo ejecutar Mentor")}finally{setBusy(false)}};
 const cycle=async()=>{if(busy)return;setBusy(true);try{const r=await backendApi.neuralMentorCycle();toast.success(r?.skipped?"Ciclo revisado: "+r.skipped:"Ciclo automático completado");await load()}catch(e:any){toast.error(e.message)}finally{setBusy(false)}};
 const approve=async(id:string)=>{try{await backendApi.neuralMentorApprove(id);toast.success("Lección verificada y convertida en conocimiento");await load()}catch(e:any){toast.error(e.message)}};
 const reject=async(id:string)=>{const reason=window.prompt("¿Por qué rechazas esta lección?","No encaja con Herencia")||"No aprobado";try{await backendApi.neuralMentorReject(id,reason);toast.success("Lección rechazada y registrada como corrección");await load()}catch(e:any){toast.error(e.message)}};
 return <div className="rounded-2xl border p-5">
  <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
   <div><div className="flex flex-wrap items-center gap-2"><h3 className="font-black text-lg">Mentor externo · Groq</h3><span className={`rounded-full px-2.5 py-1 text-[11px] font-black ${status?.connected?"bg-emerald-100 text-emerald-800":"bg-amber-100 text-amber-900"}`}>{status?.connected?`CONECTADO · ${String(status.provider||"modelo").toUpperCase()}`:"NO CONECTADO"}</span></div><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Groq actúa como profesor opcional. Propone lecciones e hipótesis, pero no controla Neural ni puede cambiar permisos. Todo lo que enseña entra como provisional hasta verificarse.</p></div>
   <div className="grid grid-cols-3 gap-2 text-center"><Mini label="Hoy" value={String(status?.trainedToday||0)}/><Mini label="Pendientes" value={String(status?.pending||0)}/><Mini label="Aprendidas" value={String(status?.approved||0)}/></div>
  </div>
  <div className="mt-5 grid gap-2 md:grid-cols-[1fr_auto_auto]"><input value={topic} onChange={e=>setTopic(e.target.value)} onKeyDown={e=>e.key==="Enter"&&void teach()} className="rounded-xl border px-4 py-3" placeholder="Tema que quieres que Groq enseñe a Neural"/><button disabled={busy||!status?.connected} onClick={()=>void teach()} className="rounded-xl bg-primary px-5 py-3 font-black text-primary-foreground disabled:opacity-50">{busy?"Aprendiendo…":"Aprender ahora"}</button><button disabled={busy||!status?.connected} onClick={()=>void cycle()} className="rounded-xl border px-4 py-3 font-bold disabled:opacity-50">Ciclo automático</button></div>
  <div className="mt-3 text-xs text-muted-foreground">Automático: {status?.automatic?"activo":"desactivado"} · límite {status?.maxDaily||0} lecciones/día · la clave Groq permanece solo en Railway y nunca llega al navegador.</div>
  <div className="mt-5 space-y-3">{items.length?items.slice(0,12).map((x:any)=><div key={x.id} className="rounded-2xl border p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap gap-2 items-center"><b>{x.content?.topic||"Lección Mentor"}</b><span className={`rounded-full px-2 py-1 text-[10px] font-black ${x.mentorStatus==="APPROVED"?"bg-emerald-100 text-emerald-800":x.mentorStatus==="REJECTED"?"bg-red-100 text-red-800":"bg-amber-100 text-amber-900"}`}>{x.mentorStatus==="APPROVED"?"CONOCIMIENTO VERIFICADO":x.mentorStatus==="REJECTED"?"RECHAZADA":"PROVISIONAL"}</span></div><p className="mt-2 text-sm">{x.content?.lesson||"Sin resumen"}</p>{(x.content?.principles||[]).length?<div className="mt-3 space-y-1">{x.content.principles.slice(0,4).map((p:any,i:number)=><div key={i} className="rounded-lg bg-muted/50 px-3 py-2 text-xs"><b>{p.scope||"general"}:</b> {p.text}</div>)}</div>:null}</div>{x.mentorStatus==="PROVISIONAL"&&<div className="flex shrink-0 gap-2"><button onClick={()=>void approve(x.id)} className="rounded-xl bg-emerald-100 px-3 py-2 text-xs font-black text-emerald-900">Verificar y aprender</button><button onClick={()=>void reject(x.id)} className="rounded-xl bg-red-50 px-3 py-2 text-xs font-black text-red-800">Rechazar</button></div>}</div></div>):<Empty text={status?.connected?"Todavía no hay lecciones Mentor. Ejecuta la primera sesión.":"Conecta un proveedor de modelo al servicio Neural para activar Mentor."}/>}</div>
 </div>
}
